...
print_templates () {
  ...
  cp -rv ${PROJECT_DIR}/templates/operator/crds ${BASE_DIR}/operator
  cp -rv ${PROJECT_DIR}/templates/operator/deploy ${BASE_DIR}/operator
  cp -rv ${PROJECT_DIR}/templates/operator/install ${BASE_DIR}/operator
}
...
if [[ ${LINUX} -eq 1 ]]; then
  ...
  SPINNAKER_PASSWORD=$(cat ${BASE_DIR}/.hal/.secret/spinnaker_password)
  yq w -i ${BASE_DIR}/operator/install/patch-profiles.yml spec.spinnakerConfig.profiles.gate.security.user.password ${SPINNAKER_PASSWORD}
  MINIO_PASSWORD=$(cat ${BASE_DIR}/.hal/.secret/minio_password)
  yq w -i ${BASE_DIR}/operator/install/patch-storage.yml spec.spinnakerConfig.config.persistentStorage.s3.secretAccessKey ${MINIO_PASSWORD}
  ENDPOINT=$(cat ${BASE_DIR}/.hal/public_endpoint)
  yq w -i ${BASE_DIR}/operator/install/patch-security.yml spec.spinnakerConfig.config.security.apiSecurity.overrideBaseUrl https://${ENDPOINT}/api/v1
  yq w -i ${BASE_DIR}/operator/install/patch-security.yml spec.spinnakerConfig.config.security.uiSecurity.overrideBaseUrl https://${ENDPOINT}

  kubectl apply -f ${BASE_DIR}/operator/install/service-account.yml
